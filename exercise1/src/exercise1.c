...
#define MORSE_MSG_OVERFLOW_ERR    "Maximum message length exceeded. Message ignored.\rType in the termination sequence to translate the 'excess' characters that went above the array limit and caused overflow.\r\r\n"
#define MORSE_MSG_OVERFLOW_ERR_LEN 175
...
static void TaskMorseMsgProcessor( void *pvParameters ){
	...
	for( ; ; ){
		...
		while(no_of_characters_read<TASK_PROCESS_BUFFER){
			...
			/***********************************************/
			while(xQueueReceive(xQueue_12, &(read_from_queue12_value[no_of_characters_read++]), portMAX_DELAY) != pdTRUE);
//			xil_printf("Received %d characters\n", no_of_characters_read);

			if (	no_of_characters_read >= 3 &&
...
//				xil_printf("Received terminal sequence\n");
		...
		if (!break_loop){
			...
			/***********************************************/
			strcpy(read_from_queue12_value, MORSE_MSG_OVERFLOW_ERR);
			output_length = MORSE_MSG_OVERFLOW_ERR_LEN;
			error_flag = 1;
		}
		...
		char* output = error_flag ? read_from_queue12_value : output_text_sequence;

		for (int i = 0; i < output_length; i++) {
			xQueueSendToBack(xQueue_23, &(output[i]), portMAX_DELAY);
		}
	}
}
